  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-12 md:py-16">
      <div className="text-center mb-8">
        ...
        <p className="text-white/90 mt-2">Pilih kategori usia. Soal dan pembahasan disesuaikan.</p>
      </div>
      ...
      <Tabs value={age} onValueChange={(v) => setAge(v as AgeKey)}>
        <TabsList className="grid w-full grid-cols-3 bg-white/[0.06] border border-white/12 text-white">
          <TabsTrigger value="10-14">10–14 Tahun</TabsTrigger>
          <TabsTrigger value="15-19">15–19 Tahun</TabsTrigger>
          <TabsTrigger value="20-24">20–24 Tahun</TabsTrigger>
        </TabsList>
    ...
    return (
      <Card className="bg-white/[0.06] border-white/12 text-white">
        ...
        <CardContent className="grid gap-4">
          <p className="text-white/90">Skor kamu: {score} dari {questions.length}</p>
          <div className="grid gap-3">
            {questions.map((q, i) => {
              ...
              return (
                <div key={q.id} className="rounded-md border border-white/12 bg-white/[0.06] p-3">
                  <p className="font-medium">{i + 1}. {q.question}</p>
                  <p className={isCorrect ? "text-green-300 text-sm mt-1" : "text-red-300 text-sm mt-1"}>
                    Jawaban kamu: {op ? op.text : "—"} {isCorrect ? "(Benar)" : "(Kurang tepat)"}
                  </p>
                  <p className="text-white/90 text-sm mt-1">
                    Kunci: {q.options[correctIndex]?.text}
                  </p>
                  <p className="text-white/90 text-sm mt-1">
                    Pembahasan: {op?.explanation ?? q.options[correctIndex]?.explanation}
                  </p>
                </div>
              )
            })}
          ...
          <div className="flex gap-2">
            <Button onClick={() => { setIndex(0); setAnswers([]); setFinished(false) }} className="text-white">Ulangi</Button>
